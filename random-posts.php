...
class RandomPosts {
	...
	function execute($args='', $default_output_template='<li>{link}</li>'){
		if (!RandomPosts::check_post_plugin_library(__('Post-Plugin Library missing', 'random_posts_plugin'))) return;
		...
		if (0 < $options['limit']) {	
			...
			$check_contains = (trim($options['contains']['value']) !== '');
			...
			if ($omit_current_post) $where[] = where_omit_post($random_posts_current_ID);
			...
			if ($check_contains) $where[] = where_check_contains($options['contains']['key'], $options['contains']['op'], $options['contains']['value']);
		...
		return ($output) ? $output . sprintf("<!-- Random Posts took %.3f ms -->", 1000 * (ppl_microtime() - $start_time)) : '';
	}
...
function widget_rrm_random_posts_init() {
	...
	function widget_rrm_random_posts($args) {
		...
		if (eval($condition)) {
			...
			$output = RandomPosts::execute('limit='.$number.'&'.$widget_parameters);
			if ($output) {
				echo $before_widget;
...
function random_posts_init () {
	...
	if ($options['feed_on'] === 'true' && function_exists('ppl_register_post_filter')) ppl_register_post_filter('feed', 'random-posts', 'RandomPosts');
	...
	if ($options['append_on'] === 'true' && function_exists('ppl_register_post_filter')) ppl_register_post_filter('append', 'random-posts', 'RandomPosts', $condition);
}
