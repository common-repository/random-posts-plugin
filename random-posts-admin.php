...
function random_posts_options_page(){
	...
	_e('help and instructions', 'random_posts_plugin'); 
	echo '</a></h2></div>';
	if (!RandomPosts::check_post_plugin_library(__('<h1>Please install the <a href="http://downloads.wordpress.org/plugin/post-plugin-library.zip">Post Plugin Library</a> plugin.</h1>', 'random_posts_plugin'))) return;
...
function random_posts_filter_options_subpage(){
	...
	if (isset($_POST['update_options'])) {
		...
		if (defined('POC_CACHE_4')) poc_cache_flush();
		// Fill up the options with the values chosen...
		$options = ppl_options_from_post($options, array('excluded_posts', 'included_posts', 'excluded_authors', 'included_authors', 'excluded_cats', 'included_cats', 'tag_str', 'custom', 'contains'));
		update_option('random-posts', $options);
		// Show a message to say we've done something
		echo '<div class="updated fade"><p>' . __('Options saved', 'random_posts_plugin') . '</p></div>';
	} 
	...
	?>
		...
		<table class="optiontable form-table">
			<?php 
				...
				ppl_display_contains($options['contains']); 
			?>
		</table>
		<div class="submit"><input type="submit" name="update_options" value="<?php _e('Save Filter Settings', 'random_posts_plugin') ?>"  /></div>
		<?php if (function_exists('wp_nonce_field')) wp_nonce_field('random-posts-update-options'); ?>
...
function random_posts_placement_options_subpage(){
	...
	?>
		<div class="wrap">
		<h2><?php _e('Placement Settings', 'random_posts_plugin'); ?></h2>
		<form method="post" action="">
		<div class="submit"><input type="submit" name="update_options" value="<?php _e('Save Placement Settings', 'random_posts_plugin') ?>" /></div>
		<table class="optiontable form-table">
			<?php 
...
function random_posts_install() {
	...
	if (!isset($options['feed_parameters'])) $options['feed_parameters'] = 'prefix=<strong>'.__('Random Posts', 'random_posts_plugin').':</strong><ul class="random-posts">&suffix=</ul>';
	if (!isset($options['append_on'])) $options['append_on'] = 'false';
	if (!isset($options['append_priority'])) $options['append_priority'] = '10';
	if (!isset($options['append_parameters'])) $options['append_parameters'] = 'prefix=<h3>'.__('Random Posts', 'random_posts_plugin').':</h3><ul class="random-posts">&suffix=</ul>';
	...
	if (!isset($options['contains'])) {$options['contains']['key'] = 'post_content'; $options['contains']['op'] = 'includes'; $options['contains']['value'] = '';}
